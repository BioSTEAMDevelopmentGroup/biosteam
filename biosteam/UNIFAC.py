...
def DORTMUND(self, specie_IDs, xs, T):
    ...
    **Parameters**
    ...
    return UNIFAC_Coeffictients(self, xs, T, DOUFSG, DOUFIP2016, DORTMUND_psi, loggammacs_DORTMUND, chemgroups, rs, qs, group_counts)
...
def loggammacs_UNIFAC(qs, rs, xs, cmps):
...
def loggammacs_DORTMUND(qs, rs, xs, cmps):
    ...
    for i in cmps:
        ...
        Fi = qs_i/qsxs
        Vi2 = rs_34[i]/rsxs2
        loggammacs.append(1. - Vi2 + log(Vi2) - 5.*qs_i*(1. - Vi/Fi + log(Vi/Fi)))
    return loggammacs
...
def UNIFAC_Coeffictients(self, xs, T, subgroups, interactions, psi_function, loggammacs_function, chemgroups, rs, qs, group_counts):
    """For both DORTMUND and UNIFAC VLE and LLE"""
    cmps = range(len(xs))

    # Sum the denominator for calculating Xs
    group_sum = sum(count*xs[i]
                    for i in cmps for count in chemgroups[i].values())

    # Caclulate each numerator for calculating Xs
    group_count_xs = {}
    for group in group_counts:
        tot_numerator = sum(chemgroups[i][group]*xs[i]
                            for i in cmps if group in chemgroups[i])
        group_count_xs[group] = tot_numerator/group_sum

    loggammacs = loggammacs_function(qs, rs, xs, cmps)

    Q_sum_term = sum([subgroups[group].Q*group_count_xs[group]
                      for group in group_counts])
    ...
    for k in group_counts:
        ...
        for m in group_counts:
            ...
            sum3 = sum(area_fractions[n]*UNIFAC_psis[m][n]
    ...
    for groups in chemgroups:
        ...
        Q_sum_term = sum([subgroups[group].Q*chem_group_count_xs[group]
                         for group in groups])
        ...
        for k in groups:
            ...
            for m in groups:
                ...
                sum3 = sum(chem_area_fractions[n]
