...
__all__ = ['ModelEQ100', 'ModelEQ101', 'ModelEQ102',
           'ModelEQ104', 'ModelEQ105', 'ModelEQ106',
           'ModelEQ107', 'ModelEQ114', 'ModelEQ115', 
           'ModelEQ116', 'ModelEQ127']
...
def ModelEQ100(A=0., B=0., C=0., D=0., E=0., F=0., G=0., order=0., *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation # 100. Used in calculating the molar heat capacities of liquids and solids, liquid thermal conductivity, and solid density. Note that high-order polynomials like this may need large numbers of decimal places to avoid unnecessary error.
    ...
    return TDependentModel(DIPPR_EQ100, Tmin, Tmax)


def ModelEQ101(A, B, C, D, E, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation # 101. Used in calculating vapor pressure, sublimation pressure, and liquid viscosity. E is often an integer. As the model is exponential, a sufficiently high temperature will cause an OverflowError. A negative temperature (or just low, if fit poorly) may cause a math domain error.
    ...
    >>> ModelEQ101(300, 73.649, -7258.2, -7.3037, 4.1653E-6, 2)
    3537.44834545549
    ...
    return TDependentModel(DIPPR_EQ101, Tmin, Tmax)


def ModelEQ102(A, B, C, D, order=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation # 102. Used in calculating vapor viscosity, vapor thermal conductivity, and sometimes solid heat capacity. High values of B raise an OverflowError. C and D are often 0.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ102, Tmin, Tmax)
        

def ModelEQ104(A, B, C, D, E, order=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #104. Often used in calculating second virial coefficients of gases. C, D, and E are normally large values.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ104, Tmin, Tmax)

def ModelEQ105(A, B, C, D, Tmin, Tmax, vol=False):
    r'''Return a TDependentModel object of the DIPPR Equation #105. Often used in calculating liquid molar volume. C is sometimes the fluid's critical temperature.
    ...
    return TDependentModel(DIPPR_EQ105, Tmin, Tmax)


def ModelEQ106(Tc, A, B, C=0, D=0, E=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #106. Often used in calculating liquid surface tension, and heat of vaporization. 
    ...
    return TDependentModel(DIPPR_EQ106, Tmin, Tmax)


def ModelEQ107(A=0, B=0, C=0, D=0, E=0, order=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #107. Often used in calculating ideal-gas heat capacity. Also called the Aly-Lee equation.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ107, Tmin, Tmax)

def ModelEQ114(Tc, A, B, C, D, order=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #114. Rarely used, normally as an alternate liquid heat capacity expression.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ114, Tmin, Tmax)


def ModelEQ115(A, B, C=0, D=0, E=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #115. No major uses; has been used as an alternate liquid viscosity expression, and as a model for vapor pressure.
    ...
    return TDependentModel(DIPPR_EQ115, Tmin, Tmax)


def ModelEQ116(Tc, A, B, C, D, E, order=0, *, Tmax, Tmin):
    r'''Return a TDependentModel object of the DIPPR Equation #116. Used to describe the molar density of water fairly precisely; no other uses listed.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ116, Tmin, Tmax)


def ModelEQ127(A, B, C, D, E, F, G, order=0, *, Tmin, Tmax):
    r'''Return a TDependentModel object of the DIPPR Equation #127. Rarely used, and then only in calculating ideal-gas heat capacity.
    ...
    if order == 0:
        ...
        raise Exception(order_not_found_msg)
    return TDependentModel(DIPPR_EQ127, Tmin, Tmax)
