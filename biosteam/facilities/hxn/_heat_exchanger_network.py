# -*- coding: utf-8 -*-
# This module will be moved to a new repository called "HXN: The automated Heat Exchanger Network design package."
# Copyright (C) 2020-2025, Sarang Bhagwat <sarangb2@illinois.edu>, Yoel Cortes-Pena <yoelcortes@gmail.com>
# 
...
class HeatExchangerNetwork(bst.Facility):
    ...
    def __init__(self, ID='', T_min_app=5., units=None, ignored=None, Qmin=1e-3,
                 ...
                 transient_C_flows=False, sort_potential_matches_by_T=False,
                 heat_transfer_efficiency=None, # if not None, overwrites utility heat transfer efficiencies
                 ):
        ...
        self.transient_C_flows = transient_C_flows
        self.sort_potential_matches_by_T = sort_potential_matches_by_T
        ...
        with flowsheet.temporary(), bst.IgnoreDockingWarnings():
            if use_cached_network:
                ...
                synthesize_network(hx_utils, self.T_min_app, self.Qmin, 
                                   ...
                                   self.transient_C_flows, self.sort_potential_matches_by_T,
                ...
                self._hot_util_load = hot_util_load
                self._cold_util_load = cold_util_load
                self._H_for_T_intervals = H_for_T_intervals
            ...
            if self.heat_transfer_efficiency:
                heat_transfer_efficiency = self.heat_transfer_efficiency
                Q_bal = (
                    (2.*sum([abs(i.Q) for i in new_HXs])
                     + sum([abs(i.duty * heat_transfer_efficiency) for i in hu_sums2]))
                    / sum([abs(i.duty * heat_transfer_efficiency) for i in hu_sums1])
                )
            else:
                Q_bal = (
                    (2.*sum([abs(i.Q) for i in new_HXs])
    ...
    def _get_stream_life_cycles(self):
        ...
        SLCs = [StreamLifeCycle(index, index in cold_indices, self.original_heat_exchangers[index].ID) for index in indices]
        for SLC in SLCs:
    ...
    def plot_composite_curve(self):
        H_for_T_intervals = self._H_for_T_intervals
        pinch_T_arr = self.pinch_Ts
        hot_util_load = self._hot_util_load
        cold_util_load = self._cold_util_load
        hot_indices = self.hot_indices
        cold_indices = self.cold_indices
        
        Ts_all = []
        for k in H_for_T_intervals.keys():
            if not k[0]-273.15 in Ts_all:
                Ts_all.append(k[0]-273.15)
            if not k[1]-273.15 in Ts_all:
                Ts_all.append(k[1]-273.15)
        Ts_all.sort(key=lambda i: i)
        original_hxs = self.original_heat_exchangers
        
        # heating curve
        heating_Ts, heating_Hs = [], []
        for i in cold_indices:
            hx = original_hxs[i]
            instream, outstream = hx.ins[0], hx.outs[0]
            heating_Ts.append((instream.T-273.15, outstream.T-273.15))
            if heating_Hs:
                heating_Hs.append(heating_Hs[-1] + outstream.H-instream.H)
            else:
                heating_Hs.append(0 + outstream.H-instream.H)
        
        # cooling curve
        cooling_Ts, cooling_Hs = [], []
        for i in hot_indices:
            hx = original_hxs[i]
            instream, outstream = hx.ins[0], hx.outs[0]
            cooling_Ts.append((instream.T-273.15, outstream.T-273.15))
            if cooling_Hs:
                cooling_Hs.append(cooling_Hs[-1] + outstream.H-instream.H)
            else:
                cooling_Hs.append(0 + outstream.H-instream.H)
        
        print(heating_Ts, heating_Hs)
        print(cooling_Ts, cooling_Hs)
        for i in range(len(heating_Ts)):
            plt.plot([heating_Hs])
