...
__all__ = ('df2unit', 'xl2dct', 'xl2mod', 'xl2txt')
    ...
    for name_sim in df.columns.levels[0]:
        ...
        try:
            ...
            try: supercls = getattr(units, supername)
                ...
                raise ValueError(f"invalid simulation option '{sim}'")
        supercls = supercls.__name__
        for ID in cost_items:
            basis, S_units, S, lb, ub, CE, cost, n, kW, BM, N, lifetime = [(None if i is False else i) for i in cost_items[ID]]
            txt += (
                f"@cost('{basis}', '{ID}', S={S}, units='{S_units}',\n"
                f"      CE={CE}, cost={cost}, n={n}, BM={BM}"
            )
            if kW: txt += f", kW={round(kW, 9)}"
            line = ""
            if lb is not None: line += f", lb={lb}"
            if ub is not None: line += f", ub={ub}"
            if N is not None: line += f", N={N}"
            if lifetime is not None: line += f", lifetime={lifetime}"
            if line: txt += "\n      " + line[2:]
            txt += ')\n'
            
        txt += f"class {name}({supercls}): pass\n\n"
    return txt

def xl2df(file, sheet_name=0, stacklevel=2):
    """Return a data frame of unit subclasses from excel file."""
...
def xl2dct(file, sheet_name=0, stacklevel=2):
    ...
    df = xl2df(file, sheet_name=0, stacklevel=2)
    return df2dct(df)
